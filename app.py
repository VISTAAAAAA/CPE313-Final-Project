...
st.set_page_config(page_title="Realtime Vehicle Speed", layout="wide")
...
start_button = st.button("▶ Start processing video")
...
if video_file is not None and start_button:
    ...
    for frame_idx in range(frame_count):
        ...
        ROI = (max(0,cx-side//2), max(0,cy-side//2), min(W-1,cx+side//2), min(H-1,cy+side//2))

        crop = frame[ROI[1]:ROI[3], ROI[0]:ROI[2]]
        scale = TARGET_SIZE / crop.shape[0]
        resized = cv2.resize(crop, None, fx=scale, fy=scale)

        results = model(resized, verbose=False, device=0 if torch.cuda.is_available() else "cpu")[0]
        ...
        for tr in tracks.astype(int):
            ...
            elif tid in entry_times and tid not in speeds and abs(cy2-POINT_B_Y)<=LINE_TOL:
        ...
        progress.progress((frame_idx+1)/frame_count)

        time.sleep(0.02)  # simulate real-time processing delay (optional)
